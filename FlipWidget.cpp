...
QFlipWidget::QFlipWidget( QRectF bounds, QPixmap front, QPixmap back, bool useOpenGL, QWidget* parent )
:
...
m_backBorderBrush( this->palette().window() )
{
    ...
    //QRectF R = QRectF( (-bounds.width() / 2.0), (-bounds.height() / 2.0), 100, 100 );
    //m_frontItem = new RoundRectItem(m_bounds , m_frontBorderBrush, m_frontWidget );
    m_frontItem = new RoundRectItem(m_bounds , m_frontBorderBrush, 0 );
    m_frontItem->setPixmap(front);
    ...
    m_backItem = new RoundRectItem( m_bounds, m_backBorderBrush, 0 );
    m_backItem->setPixmap(back);
...
void QFlipWidget::mousePressEvent(QMouseEvent *event)
{
    if (isFlipped())
        return;
    if (flipping)
    ...
    if (event->button()==Qt::LeftButton)
    {
        ...
        //Flip();
        //QTimer::singleShot(GetFlipDuration()+500, this, SLOT(emitClicked()));
        emit clicked();
    }
}
...
{
    flipping=true;
    QTimer::singleShot(this->GetFlipDuration(), this, SLOT(endFlipping()));
    m_flipLeft = flipLeft;
...
void QFlipWidget::setValor(int v)
{
    Valor=v;
}

void QFlipWidget::setPixmap(bool frente, QPixmap P)
{
    if (frente)
        m_frontItem->setPixmap(P);
    else
        m_backItem->setPixmap(P);
}

void QFlipWidget::endFlipping()
{
    flipping=false;
}
